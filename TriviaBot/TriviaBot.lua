...
local TB_VERSION = C_AddOns.GetAddOnMetadata(addonName,"Version")
...
function TriviaBot.QuestionPackRegistry()
	for i = 1, C_AddOns.GetNumAddOns() do
		local questionPack = C_AddOns.GetAddOnInfo(i)
		if C_AddOns.GetAddOnEnableState(i, character) > 0 and not C_AddOns.IsAddOnLoaded(i) and C_AddOns.IsAddOnLoadOnDemand(i) then
			local valid = C_AddOns.GetAddOnMetadata(i, "X-TriviaBot-Questions")
			if valid then
	...
	if next(TriviaBot_QuestionPacks) then
		for i, v in pairs(TriviaBot_QuestionPacks) do
			...
			TriviaBot_Questions[i]['Title'] = GRAY_FONT_COLOR_CODE..C_AddOns.GetAddOnMetadata(v, "Notes")..FONT_COLOR_CODE_CLOSE
			TriviaBot_Questions[i]['Description'] = L.TB_GUI_LOD
			TriviaBot_Questions[i]['Author'] = C_AddOns.GetAddOnMetadata(v, "Author")
...
function TriviaBot.GUIInitialize()
	...
	local maxLength = 15  -- Set the maximum width for the text
	...
	-- Question List
	TriviaBotGUI.QuestionList = LibDD:Create_UIDropDownMenu("TriviaBotGUI_QuestionList", TriviaBotGUI)
	TriviaBotGUI.QuestionList.OnClick = function(self)
		...
		if TriviaBot_Questions[set]['Stub'] then
			...
			if questionPack then
				local loaded, reason = C_AddOns.LoadAddOn(questionPack)
				if not loaded then
		...
		local titleText = TB_Question_Sets[TriviaBot_Config['Question_Set']]['Title']
		if string.len(titleText) > maxLength then
			titleText = string.sub(titleText, 1, maxLength) .. "..."
		end
		
		LibDD:UIDropDownMenu_SetSelectedValue(TriviaBotGUI.QuestionList, TriviaBot_Config['Question_Set'])
		LibDD:UIDropDownMenu_SetText(TriviaBotGUI.QuestionList, titleText)
		LibDD:UIDropDownMenu_SetSelectedValue(TriviaBotGUI.CategoryList, 0)
		LibDD:UIDropDownMenu_SetText(TriviaBotGUI.CategoryList, L.TB_GUI_ALL)
	end
	TriviaBotGUI.QuestionList.Initialize = function()
		...
		for id,_ in pairs(TB_Question_Sets) do
			...
			info.value = id;
			info.text = TB_Question_Sets[id]['Title'];
	...
	TriviaBotGUI.QuestionList:SetWidth(150);
	LibDD:UIDropDownMenu_SetButtonWidth(TriviaBotGUI.QuestionList, 20);
	TriviaBotGUI.QuestionList:SetPoint("TOPLEFT", TriviaBotGUI, "TOP", -35, -20);
	LibDD:UIDropDownMenu_Initialize(TriviaBotGUI.QuestionList, function(self, ...) TriviaBotGUI.QuestionList:Initialize(self, ...) end)
	-- Question List Label
	...
	-- Category List
	TriviaBotGUI.CategoryList = LibDD:Create_UIDropDownMenu("TriviaBotGUI_CategoryList", TriviaBotGUI)
	TriviaBotGUI.CategoryList.OnClick = function(self)
		...
		LibDD:UIDropDownMenu_SetSelectedValue(TriviaBotGUI.CategoryList, TriviaBot_Config['Question_Category']);
	end
	TriviaBotGUI.CategoryList.Initialize = function()
		...
		if (#TB_Question_Sets > 0 and TB_Question_Sets[set]) then
			...
			LibDD:UIDropDownMenu_SetSelectedValue(TriviaBotGUI.CategoryList, cat);
			if tonumber(cat) == 0 then
				LibDD:UIDropDownMenu_SetText(TriviaBotGUI.CategoryList, L.TB_GUI_ALL)
			else
				LibDD:UIDropDownMenu_SetText(TriviaBotGUI.CategoryList, TB_Question_Sets[set]['Categories'][TriviaBot_Config['Question_Category']])
			end
		end
	end
	TriviaBotGUI.CategoryList:ClearAllPoints();
	TriviaBotGUI.CategoryList:SetWidth(150);
	LibDD:UIDropDownMenu_SetButtonWidth(TriviaBotGUI.CategoryList, 20);
	TriviaBotGUI.CategoryList:SetPoint("TOPLEFT", TriviaBotGUI.QuestionList, "BOTTOMLEFT", 0, 0);	
	LibDD:UIDropDownMenu_Initialize(TriviaBotGUI.CategoryList, function(self, ...) TriviaBotGUI.CategoryList:Initialize(self, ...) end)
	-- Category List Label
	...
	-- Chat Type List
	TriviaBotGUI.ChatType = LibDD:Create_UIDropDownMenu("TriviaBotGUI_ChatType", TriviaBotGUI)
	TriviaBotGUI.ChatType.OnClick = function(self)
		...
		LibDD:UIDropDownMenu_SetSelectedValue(TriviaBotGUI.ChatType, TriviaBot_Config['Chat_Type']);
	end
	...
	TriviaBotGUI.ChatType:SetWidth(150);
	LibDD:UIDropDownMenu_SetButtonWidth(TriviaBotGUI.ChatType, 20);
	TriviaBotGUI.ChatType:SetPoint("TOPLEFT", TriviaBotGUI.CategoryList, "BOTTOMLEFT", 0, 0);
	LibDD:UIDropDownMenu_Initialize(TriviaBotGUI.ChatType, function(self, ...) TriviaBotGUI.ChatType:Initialize(self, ...) end)
	...
	if C_AddOns.IsAddOnLoaded("TriviaBot_QuestionMaker") then
	...
	TriviaBotGUI.StartStopToggle = function()
		if (TB_Running) then
			...
			LibDD:UIDropDownMenu_Disable(TriviaBotGUI.ChatType);
			...
			TriviaBotGUI.StartStopButton:SetText(L.TB_GUI_STARTTRIVIA);
			TriviaBotGUI.SkipButton:Disable();
			LibDD:UIDropDownMenu_Enable(TriviaBotGUI.ChatType);
